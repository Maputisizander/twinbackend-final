...
5. [Skycable — Nodes](#5-skycable--nodes) *(includes `/pole-photos`)*
6. [Skycable — Poles](#6-skycable--poles) *(slots auto-seeded C1–C5, DA)*
7. [Skycable — Spans](#7-skycable--spans)
8. [Skycable — Teardown Reports](#8-skycable--teardown-reports) *(per-pole photo fields)*
...
**Response:** Same shape as `GET /poles/{id}` — includes teardown photos aggregated from all spans the pole appears in:
```json
{
  "id": 10,
  "pole_code": "SKY-001",
...
### GET `/poles/map`
Returns all SkycablePoles that have GPS coordinates. Optimised for map rendering — no pagination.
...
### GET `/poles/all`
Returns **all** SkycablePoles — both with and without GPS. Use this for the sidebar list in the pole map view. Includes a `has_gps` flag so the frontend can split map pins vs. list-only entries.
...
|---|---|---|
| `slot_label` | ✅ | `C1` \| `C2` \| `C3` \| `C4` \| `C5` \| `DA` \| any custom string (max 50) |
...
> All images are auto-resized to ≤ 1280px and compressed to JPEG 75% quality server-side.
...
| `from_before_photo` | ❌ | From-pole: before teardown photo, image file, max 10MB |
| `from_after_photo` | ❌ | From-pole: after teardown photo, image file, max 10MB |
| `from_pole_tag_photo` | ❌ | From-pole: pole tag photo, image file, max 10MB |
| `to_before_photo` | ❌ | To-pole: before teardown photo, image file, max 10MB |
| `to_after_photo` | ❌ | To-pole: after teardown photo, image file, max 10MB |
| `to_pole_tag_photo` | ❌ | To-pole: pole tag photo, image file, max 10MB |
| `bunching_photo` | ❌ | Span-wide bunching photo, image file, max 10MB |
| `notes` | ❌ | string |
| `slots` | ❌ | array — cable slots cleared during this teardown |
| `slots[].pole_id` | ✅ if slots | `poles.id` |
| `slots[].pole_cable_slot_id` | ✅ if slots | `pole_cable_slots.id` |
| `slots[].slot_label` | ✅ if slots | e.g. `C4` |
...
**Base:** `/api/v1` — no company prefix  
**Auth:** `auth:sanctum` only (no company check — Globe field staff submit Skycable teardowns)  
...
| Field | Required | Notes |
|---|---|---|
| `pole_span_id` | ✅ | `skycable_spans.id` |
| `started_at` | ❌ | ISO date string (PHT recommended) |
| `finished_at` | ❌ | ISO date string |
| `captured_at_device` | ❌ | ISO date string |
| `did_collect_all_cable` | ❌ | `"1"` or `"0"` string |
| `collected_cable` | ❌ | numeric (meters actually collected) |
| `recovered_cable` | ❌ | numeric (fallback if `collected_cable` absent) |
| `unrecovered_cable` | ❌ | numeric |
| `unrecovered_reason` | ❌ | string |
| `expected_cable` | ❌ | numeric |
| `gps_latitude` / `gps_longitude` | ❌ | numeric |
| `from_pole_latitude` / `from_pole_longitude` | ❌ | numeric (preferred over gps_*) |
| `to_pole_latitude` / `to_pole_longitude` | ❌ | numeric |
| `from_before` | ❌ | file — from-pole before photo (max 15MB) |
| `from_after` | ❌ | file — from-pole after photo |
| `from_tag` | ❌ | file — from-pole tag photo |
| `to_before` | ❌ | file — to-pole before photo |
| `to_after` | ❌ | file — to-pole after photo |
| `to_tag` | ❌ | file — to-pole tag photo |
| `before_span` | ❌ | file — span/bunching photo |

**Photo field → `photo_type` mapping:**
